...
- [[ERC-721 非同質化代幣標準]] —— **規格層**：本卡第 9 節的 ERC721 介面是 2018 **提案版**（`deed`／`deedUri`／`countOfOwners`），現行定案版介面不同（`tokenURI`／`safeTransferFrom`／兩層授權）。實作一律以那張為準。
- [[ERC-1155 多代幣標準]] —— **本卡明確標示全章 0 命中**的多代幣標準，缺口補在那張。
